...
export class CategoriesService {
  ...
  getAllCategories(
    ...
    if (selectedStatus !== null && selectedStatus !== undefined)
      requestBody.status = Boolean(selectedStatus);
    return this.http.post<IAllCategory>(
      `${WEB_SITE_BASE_URL}categories`,
      requestBody,
    );
  }
  getCategoryById(CategoryId: string) {
    return this.http.get<ICategoryById>(
      `${WEB_SITE_BASE_URL}categories/${CategoryId}`,
    );
  }
  addCategory(categoryData: FormData) {
    return this.http.post<IAddCategoryResponse>(
      `${WEB_SITE_BASE_URL}categories/create`,
      categoryData,
    );
  }
  updateCategory(CategoryId: number, categoryData: FormData) {
    return this.http.post<IUpdateCategoryResponse>(
      `${WEB_SITE_BASE_URL}categories/${CategoryId}`,
      categoryData,
    );
  }
  destroyCategory(CategoryId: string) {
    return this.http.post<IToggleCategory>(
      `${WEB_SITE_BASE_URL}category_destroy/${CategoryId}`,
      {},
    );
  }
  enableCategory(CategoryId: string) {
    return this.http.post<IToggleCategory>(
      `${WEB_SITE_BASE_URL}categories/${CategoryId}/toggle`,
      {},
    );
  }
}

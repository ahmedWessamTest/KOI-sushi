...
export class GChoicesAddComponent {
  ...
  ngOnInit() {
    if (this.activatedRoute.snapshot.data["products"]) {
      if (this.router.url.includes("edit")) {
        ...
        this.submitForm.patchValue(JSON.parse(localStorage.getItem("choices")!));
      }
  ...
  saveForm() {
    ...
    if (this.isEditing && this.productId) {
      this.productsService.updateProductChoices(this.piceDetails.id.toString(), pricePriceData).subscribe(() => {
      ...
      this.productsService.storeProductChoices(this.productId!, pricePriceData).subscribe(() => {
        this.router.navigate(["/dashboard/menu/products/products-choice/" + this.productId]);

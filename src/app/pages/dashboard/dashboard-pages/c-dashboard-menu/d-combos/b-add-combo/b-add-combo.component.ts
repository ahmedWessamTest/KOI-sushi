...
@Component({
  ...
  imports: [ButtonModule,
    ...
    DropdownModule,
    CommonModule,
    OnlyNumberDirective, MultiSelectModule],
...
export class BAddComboComponent {
  ...
  private initializeForm() {
    this.submitForm = this.fb.group({
      ...
      combo_categories: this.fb.array([], [Validators.required, Validators.minLength(1)]),
    });
  }
  ...
  ngOnInit() {
    ...
    if (this.activatedRoute.snapshot.data["combo"]) {
      ...
      if (combo.combo_categories && combo.combo_categories.length > 0) {
        ...
        combo.combo_categories.forEach((cat: any, index: number) => {
          ...
          group.patchValue({
            ...
            limit: cat.limit,
            combo_category_excludes: cat.combo_category_excludes ? cat.combo_category_excludes.map((e: any) => e.product_id) : []
          });
      ...
      if (combo.price === null || combo.price === 0) { // Check if price is optional/zero
...
onCategoryChange(categoryId: number, index: number, isInitialLoad: boolean = false) {
  ...
  this.productsService.getAllProducts(0, 0, '', [categoryId]).pipe(
    ...
    error: () => {this.ngxSpinnerService.hide("loadingProducts" + index);this.loadingProducts = false;}
  });
}

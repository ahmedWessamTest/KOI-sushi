...
export class BOrdersHistoryComponent implements OnInit {
  orders: OrderData[] = [];
    ...
    statusSteps = ['requested', 'preparing', 'out_for_delivery', 'delivered','canceled','failed'];
    searchValue: string = '';
    ...
    loadOrders() {
      this.ordersService.getOrdersHistory(this.currentPage,this.rowsPerPage,this.selectedRange.startDate,this.selectedRange.endDate).subscribe({
        next: (data) => {
          this.allOrders = data
    ...
    getStatusClass(status: string): string {
      switch (status) {
        ...
        case 'failed':
          return 'bg-red-200 text-red-800'; // Light Green
        case 'canceled':
          return 'bg-red-200 text-red-800'; // Light Green
    ...
    const statusWithIcons = [
      ...
      { label: 'Canceled', value: 'canceled', icon: 'pi pi-times-circle text-red-500' },

...
export class BVoucherIdComponent implements OnInit, OnDestroy {
    ...
    this.submitForm
      ...
      .subscribe((selectedCatIds: number[]) => {
        ...
        if (currentProductIds?.length > 0) {
          const cleanedIds = this.allProducts()
            .filter(
              (p) =>
          ...
          if (cleanedIds.length !== currentProductIds.length) {
            this.submitForm
              .get('products_ids')
              ?.setValue(cleanedIds, { emitEvent: false });
          }
        }
        this.fetchProducts();
      });
  ...
  checkEditMode(): void {
    ...
    if (voucherData) {
      ...
      const catIds =
        voucher.voucher_categories?.map((item: any) => item.category_id) || [];
      const prodIds =
        voucher.voucher_categories?.flatMap((cat: any) =>
      ...
      this.submitForm.patchValue({
        ...
        expiration_date: voucher.expiration_date
          ? new Date(voucher.expiration_date)
          : '',
  ...
  saveForm(): void {
    ...
    if (categoriesPayload.length > 0) {
      categoriesPayload.forEach((item: any, index: number) => {
        // إرسال ID التصنيف
        formData.append(
          `voucher_categories[${index}][category_id]`,
          String(item.category_id),
        );

        // إرسال مصفوفة المنتجات المستثناة داخل هذا التصنيف
        if (
          item.voucher_category_excludes &&
          item.voucher_category_excludes.length > 0
        ) {
          item.voucher_category_excludes.forEach(
            (productId: number, pIndex: number) => {
              formData.append(
                `voucher_categories[${index}][voucher_category_excludes][${pIndex}]`,
                String(productId),
              );
            },
          );

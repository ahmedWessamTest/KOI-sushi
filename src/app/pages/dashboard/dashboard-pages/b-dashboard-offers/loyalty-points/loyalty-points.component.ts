...
import { Category, Product } from '../../../../../core/Interfaces/b-combo/IGetAllComboProducts';
   ...
   submitForm!: FormGroup;
     ...
     initializeForm(): void {
       this.submitForm = this.fb.group({
         ...
         spend_percentage: [0, [Validators.required, Validators.min(0),Validators.max(100)]],
     ...
     patchForm(loyaltySettings:LoyaltyPointSetting): void {
         const catIds = loyaltySettings.loyalty_point_categories?.map((item: any) => item.category_id) || [];
   
         const prodIds = loyaltySettings.loyalty_point_categories?.flatMap((catItem: any) =>
           catItem.loyalty_point_category_excludes?.map((excludeItem: any) => excludeItem.product_id)
         ) || [];

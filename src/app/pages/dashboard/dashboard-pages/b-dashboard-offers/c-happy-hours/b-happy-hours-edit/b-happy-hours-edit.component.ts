...
import { debounceTime, distinctUntilChanged, finalize, of, startWith, Subject, switchMap, takeUntil, tap, timer } from "rxjs";
...
@Component({
  ...
  standalone: true,
  imports: [ButtonModule, CardModule, InputSwitchModule, DialogModule, FormsModule, ReactiveFormsModule, RouterLink, ToastModule, MultiSelectModule],
...
allProducts = signal<Product[]>([]);
 ...
 private _ProductsService = inject(ProductsService);
  ...
  checkEditMode(): void {
    const happyHourData: HappyHour =this.activatedRoute.snapshot.data["happyHours"].happy_hour;
    
    if (happyHourData) {
      this.happyHoursById = happyHourData.id
      this.isEditing = true;

      const catIds = happyHourData.happy_hour_categories?.map((item: any) => item.category_id) || [];

      const prodIds = happyHourData.happy_hour_categories?.flatMap((catItem: any) =>
        catItem.happy_hour_category_excludes?.map((excludeItem: any) => excludeItem.product_id)
      ) || [];
  ...
  saveForm() {
    ...
    const selectedProductsObjects = this.allProducts().length > 0 ? this.allProducts().filter(p =>
    ...
    const happyHourData = {
      ...
      this.happyHoursService.updateHappyHoursById(happyHourData,this.happyHoursById).subscribe({next:()=>{
